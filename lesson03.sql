...
CREATE TABLE city(
	id SERIAL,
	name VARCHAR(20),
	INDEX city_name_idx(name) -- можно писать город самостоятельно
);
...
CREATE TABLE users_city(
	...
	PRIMARY KEY (user_id, city_id), -- чтобы не было 2 записей о пользователе и городе рождения
    FOREIGN KEY (user_id) REFERENCES users(id),
...
CREATE TABLE `work`(
	id SERIAL,
	name VARCHAR(100),
	INDEX work_name_idx(name) -- можно писать свою работу самостоятельно
);
...
CREATE TABLE users_work(
	...
	`start` DATE NOT NULL,
	finish DATE, -- может быть пустым, говоря о том что пользователь работает по сей день
  
	PRIMARY KEY (user_id, work_id), -- чтобы не было 2 записей о пользователе и месте его работы
    FOREIGN KEY (user_id) REFERENCES users(id),
